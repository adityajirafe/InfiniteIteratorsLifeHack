...
function ArrowDownComponent(iconProps, theme) {
  return (
    ...
    >
      <ArrowDown stroke={iconProps.className.includes('Mui-disabled') ? alpha('#6F767E', 0.3) : '#6F767E'} />
    </IconButton>
  );
}
    ...
    components: {
      ...
      MuiPagination: {
        defaultProps: {
          ...
          renderItem: (item) => (
            <PaginationItem
              components={{ previous: (props) => Previous(props, theme), next: (props) => Next(props, theme) }}
              {...item}
            />
          )
        },
